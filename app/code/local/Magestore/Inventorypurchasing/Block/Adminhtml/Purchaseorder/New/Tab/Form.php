...
class Magestore_Inventorypurchasing_Block_Adminhtml_Purchaseorder_New_Tab_Form extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        $default_warehouse_id = Mage::getModel('inventoryplus/warehouse')
                ->getCollection()
                ->setPageSize(1)->setCurPage(1)->getFirstItem()
                ->getId();
		$warehousePlugin = (int)Mage::helper('core')->isModuleEnabled('Magestore_Inventorywarehouse');			
        ...
        /** no supplier **/
        /** @var Magestore_Inventorypurchasing_Helper_Supplier $supplierHelper */
        $supplierHelper = Mage::helper('inventorypurchasing/supplier');
        if (count($supplierHelper->returnArrAllRowOfcolumnOftableSupplier('supplier_name')) == 0) {
            $fieldset = $form->addFieldset('purchaseorder_form', array(
                'legend' => Mage::helper('inventorypurchasing')->__('Select Supplier & Warehouses')
            ));
            $fieldset->addField('pinput_hidden', 'hidden', array(
                    ...
                    <div id="messages">
                        <ul class="messages">
                            <li class="error-msg">
                                <ul>
                                    <li>
                                        <span>' . $this->__("You don't have any supplier or your all suppliers are disable!"). '</span>
                                        <span>Please <a href="'.$this->getUrl('*/inpu_supplier/index').'">click here</a> to check!</span>
                                    </li>
                                </ul>
                            </li>
                        </ul>
        ...
        if(!isset($data['warehouse_ids']) || !$data['warehouse_ids']){
            $data['warehouse_ids'] = explode(',', $this->getRequest()->getParam('warehouse_ids'));
        }
            ...
            'after_element_html' => '<script type="text/javascript">
                var productTemplateSyntax = /(^|.|\r|\n)({{(\w+)}})/;                   
                    function setSettings(urlTemplate, setElement) {
                        ...
                        var warehouse_plugin = '.$warehousePlugin.';
                        ...
                        if(!warehouse_ids && !warehouse_plugin){
                             warehouse_ids= \''.$default_warehouse_id.'\';
                        }
						if(!warehouse_ids && warehouse_plugin){
        ...
        $form->setValues($data);
        if(!isset($data['warehouse_ids']) || !$data['warehouse_ids']){
            $form->addValues(array('warehouse_ids' => Mage::getModel('inventoryplus/warehouse')->getCollection()->setPageSize(1)->setCurPage(1)->getFirstItem()->getId()));
        }
        return parent::_prepareForm();
    }
