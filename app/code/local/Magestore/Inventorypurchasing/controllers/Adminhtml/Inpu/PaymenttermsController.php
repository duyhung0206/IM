 ...
 */
class Magestore_Inventorypurchasing_Adminhtml_Inpu_PaymenttermsController extends Magestore_Inventoryplus_Controller_Action {
     ...
     * @var string
     */
    protected $_menu_path = 'inventoryplus/settings/purchase_order/paymentterms';
    ...
    protected function _initAction() {
        $this->loadLayout()
                ...
                ->_addBreadcrumb(
                        Mage::helper('adminhtml')->__('Manage Payment Terms'), Mage::helper('adminhtml')->__('Manage Payment Terms')
        );
    ...
    public function gridAction() {
        ...
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('inventorypurchasing/adminhtml_paymentterm_grid')->toHtml()
        );
    }
    ...
    public function editAction() {
        ...
        if ($model->getId() || $paymentTermId == 0) {
            ...
            $this->_addBreadcrumb(
                    Mage::helper('adminhtml')->__('Manage Payment Term'), Mage::helper('adminhtml')->__('Manage Payment Term')
            );
            $this->_addBreadcrumb(
                    Mage::helper('adminhtml')->__('Payment Term News'), Mage::helper('adminhtml')->__('Payment Term News')
            );
        ...
        if (!is_array($paymentTermIds)) {
            ...
            try {
                foreach ($paymentTermIds as $paymentTermId) {
                    $supplier = Mage::getModel('inventorypurchasing/paymentterm')->setId($paymentTermId);
                    $supplier->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('adminhtml')->__('Total %d record(s) were successfully deleted', count($paymentTermIds))
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
