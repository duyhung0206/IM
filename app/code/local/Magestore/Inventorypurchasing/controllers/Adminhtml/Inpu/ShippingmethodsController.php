 ...
 */
class Magestore_Inventorypurchasing_Adminhtml_Inpu_ShippingmethodsController extends Magestore_Inventoryplus_Controller_Action {
     ...
     * @var string
     */
    protected $_menu_path = 'inventoryplus/settings/purchase_order/shippingmethod';
    ...
    protected function _initAction() {
        $this->loadLayout()
                ...
                ->_addBreadcrumb(
                        Mage::helper('adminhtml')->__('Manage Shipping Methods'), Mage::helper('adminhtml')->__('Manage Shipping Methods')
        );
    ...
    public function gridAction() {
        ...
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('inventorypurchasing/adminhtml_shippingmethod_grid')->toHtml()
        );
    }
    ...
    public function editAction() {
        ...
        if ($model->getId() || $shippingMethodId == 0) {
            ...
            $this->_addBreadcrumb(
                    Mage::helper('adminhtml')->__('Manage Shipping Method'), Mage::helper('adminhtml')->__('Manage Shippineg Method')
            );
            $this->_addBreadcrumb(
                    Mage::helper('adminhtml')->__('Shipping Method News'), Mage::helper('adminhtml')->__('Shipping Method News')
            );
...
//                $model = $model->load($this->getRequest()->getParam('id'));
        ...
        if (!is_array($shippingMethodIds)) {
            ...
            try {
                foreach ($shippingMethodIds as $shippingMethodId) {
                    $supplier = Mage::getModel('inventorypurchasing/shippingmethod')->setId($shippingMethodId);
                    $supplier->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('adminhtml')->__('Total %d record(s) were successfully deleted', count($shippingMethodIds))
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('inventoryplus/settings/purchase_order/shippingmethod');
    }
