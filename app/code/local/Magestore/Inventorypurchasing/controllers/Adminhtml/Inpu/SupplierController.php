 ...
 */
class Magestore_Inventorypurchasing_Adminhtml_Inpu_SupplierController extends Magestore_Inventoryplus_Controller_Action {
     ...
     * @var string
     */
    protected $_menu_path = 'inventoryplus/purchaseorder/supplier';
    ...
    public function gridAction() {
        ...
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('inventorypurchasing/adminhtml_supplier_grid')->toHtml()
        );
    }
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                if (isset($data['supplier_products'])) {
                    ...
                    if (count($supplierProductsExplodes) <= 100) {
                        Mage::helper('inventoryplus')->parseStr(urldecode($data['supplier_products']), $supplierProducts);
                    } else {
                        foreach ($supplierProductsExplodes as $supplierProductsExplode) {
                            $supplierProduct = array();
                            Mage::helper('inventoryplus')->parseStr($supplierProductsExplode, $supplierProduct);
                    ...
                    if (count($supplierProducts)) {
                        ...
                        foreach ($supplierProducts as $pId => $enCoded) {
                            ...
                            if ($supplierProductItem->getId()) {
                                /* Do not update product cost in this action */
                                /** 
                                $product = Mage::getSingleton('catalog/product')->load($supplierProductItem->getProductId());
                                if($product){
                                    $product->setCost($supplierProductItem->getCost());
                                    $product->save();
                                }
                                 */
                                $countSqlOlds++;
    ...
    public function deleteAction() {
        if ($this->getRequest()->getParam('id') > 0) {
            /** get purchase order this supplier that is not completed or canceled */
            $statusCannotDeleteSupplier = Mage::getModel('inventorypurchasing/purchaseorder')->getStatusCannotDeleteSupplier();
            /** @var Magestore_Inventorypurchasing_Model_Mysql4_Purchaseorder_Collection $purchaseOrderCollection */
            $purchaseOrderCollection = Mage::getResourceModel('inventorypurchasing/purchaseorder_collection');
            $purchaseOrderCollection->addFieldToFilter('supplier_id', $this->getRequest()->getParam('id'))
                ->addFieldToFilter('status', array('in' => $statusCannotDeleteSupplier));
            if ($purchaseOrderCollection->getSize()) {
                Mage::getSingleton('adminhtml/session')->addNotice(Mage::helper('inventorypurchasing')->__('You cannot delete this supplier because have some purchase order with this supplier that is not completed!'));
    ...
    public function massDeleteAction() {
        ...
        if (!is_array($supplierIds)) {
            ...
            /** @var Magestore_Inventorypurchasing_Model_Mysql4_Purchaseorder_Collection $purchaseOrderCollection */
            $purchaseOrderCollection = Mage::getResourceModel('inventorypurchasing/purchaseorder_collection');
            $purchaseOrderCollection->addFieldToFilter('supplier_id', array('in' => $supplierIds))
                ->addFieldToFilter('status', array('in' => $statusCannotDeleteSupplier));
            $supplierCannotDeleteIds = array();
            if ($purchaseOrderCollection->getSize()) {
                foreach ($purchaseOrderCollection as $purchaseOrder) {
                    $supplierCannotDeleteIds[] = $purchaseOrder->getSupplierId();
                }
            }
            try {
                $count = 0;
                foreach ($supplierIds as $supplierId) {
                    if (!in_array($supplierId, $supplierCannotDeleteIds)) {
                        $count++;
                        $supplierModel = Mage::getModel('inventorypurchasing/supplier')->setId($supplierId);
                        $supplierModel->delete();
                    }
                }
                if (count($supplierCannotDeleteIds)) {
                    Mage::getSingleton('adminhtml/session')->addNotice(
                        Mage::helper('adminhtml')->__('You cannot delete some suppliers because have some purchase order with these suppliers that is not completed!', $count)
                    );
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
    ...
    public function getTitleByField($field) {
        $fieldArray = array(
            ...
            'supplier_name' => Mage::helper('inventorypurchasing')->__('Supplier Name '),
            'contact_name' => Mage::helper('inventorypurchasing')->__('Contact Person'),
            'email' => Mage::helper('inventorypurchasing')->__('Email'),
            'supplier_email' => Mage::helper('inventorypurchasing')->__('Email'),
    ...
    public function noSupplyProductAction() {
        $this->loadLayout()->_setActiveMenu('inventoryplus/stock_onhand/nosupplierproduct')
                ->_addBreadcrumb(
                        Mage::helper('adminhtml')->__('Items Manager'), Mage::helper('adminhtml')->__('Item Manager')
        );
        $this->_title($this->__('Inventory'))
                ->_title($this->__('Manage Non-Supplier Products'));
        $this->renderLayout();
    }

    public function noSupplyProductGridAction() {
